<#
.SYNOPSIS
  Vorbereitung von PrÃ¼fungsrechnern fÃ¼r die AP 1 (robuster Flow mit Registry/COM-Fallback).

.DESCRIPTION
  - Optionaler Nuera/NÃ¼ra-Download und Ablage auf Desktop.
  ...
  - Schnellzugriff-Dateien (.officeUI) Ã¼bernehmen.
...
.PARAMETER Nuera
  Switch: lÃ¤dt die neueste Nuera/NÃ¼ra und kopiert sie auf den Desktop.

.PARAMETER ExcelListPath
  Pfad zu AP1-TN.xlsx (Default: $PSScriptRoot\AP1-TN.xlsx)

.PARAMETER CsvFallbackPath
  Optionaler CSV-Fallback (2 Spalten: Account;Kandidat). Wird genutzt, wenn Excel-COM nicht verfÃ¼gbar ist.

.PARAMETER MaxRows
  Limit fÃ¼r einzulesende Zeilen (Default: 500)

.PARAMETER Quiet
  Minimiert Konsolenausgabe (Transcript lÃ¤uft weiter).

.PARAMETER RegistryOnly
  Erzwingt Registry-only-Modus (kein COM). Standard: Auto (wird aktiv, wenn COM-Start fehlschlÃ¤gt).
...
Write-Host "Das Skript lÃ¤uft auf Laufwerk: $scriptDrive (Pfad: $script:ScriptRoot)"
...
function Set-RegistryValues {
  ...
  # FÃ¼r kleinere Sets verwenden wir normale Schleifen statt Parallelisierung
  # da der Overhead grÃ¶ÃŸer als der Nutzen wÃ¤re
...
Write-Info "PrÃ¼fe Schreibzugriffe..."
...
function Expand-ZipWithShell {
    ...
    try {
        ...
        if (-not $zip -or -not $dest) { throw "Shell.Application konnte Pfade nicht Ã¶ffnen." }
        $dest.CopyHere($zip.Items(), 16) # 16 = Keine Fortschrittsanzeige
        # Warten bis alle Dateien kopiert wurden (grob, fÃ¼r kleine ZIPs ausreichend)
...
function Test-ExpandArchive {
  ...
  try {
    ...
    try {
      ...
      Write-Warning "Bitte prÃ¼fen Sie das entpackte NÃ¼ra-Verzeichnis manuell auf korrekte Umlaute und VollstÃ¤ndigkeit!"
      return $false
    }
  }
}
...
function Get-LatestNueraFile {
  ...
  # AufrÃ¤umen Ã„ltere Artefakte
  Get-ChildItem -Path $DownloadPath -Force -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -match '^(nuera|nÃ¼ra).*\.zip$' -or ($_.PSIsContainer -and $_.Name -match '^(nuera|nÃ¼ra)')
  ...
  try {
    ...
    $links = ($html.Links | Where-Object { $_.href -match '(nu[eÃ¼]ra\d{4}_[fh]\.zip)$' }) | Select-Object -ExpandProperty href
  } catch {
    Write-Warning "Konnte Download-Seite nicht laden: $($_.Exception.Message)"
    return
  }

  if (-not $links -or $links.Count -eq 0) { Write-Warning "Keine nuera/nÃ¼ra-Links gefunden."; return }
  ...
  foreach ($lnk in $links) {
    ...
    } catch { Write-Info "Nicht verfÃƒÂ¼gbar: $fileName" }
  }
  if ($fileInfos.Count -eq 0) { Write-Warning "Keine gÃ¼ltigen Dateien mit Ã„nderungsdatum gefunden."; return }

  $latest = $fileInfos | Sort-Object LastModified -Descending | Select-Object -First 1
  Write-Info ("Lade herunter: {0} (GeÃ¤ndert am {1})" -f $latest.FileName, $latest.LastModified)
  ...
  if (Test-ExpandArchive -ZipPath $zipPath -Destination $DownloadPath) {
    ...
    $folders = Get-ChildItem -Path $DownloadPath -Directory | Where-Object { $_.Name -match '^(nuera|nÃ¼ra)' }
...
function Initialize-OfficeApps {
  ...
  if ($firstRun) {
    Write-Host -ForegroundColor Yellow "\nBitte bestÃ¤tigen Sie ggf. alle Office-Hinweisfenster (z.B. Lizenz, Datenschutz, Willkommen) und klicken Sie auf OK.\nErst danach bitte eine beliebige Taste drÃ¼cken, damit das Skript fortfÃ¤hrt."
...
function Set-DefaultSavePaths {
  ...
  if ($UseCom) {
    ...
    Write-Info "COM Ã¼bersprungen (Registry-only)."
  }
}
  ...
  foreach ($file in @('Excel.officeUI','Word.officeUI')) {
    ...
    if (Test-Path $src) {
      try {
        ...
        Write-Info "Schnellzugriff Ã¼bernommen: $file"
...
function Copy-WordTemplate {
  ...
  if (-not (Test-Path $sourcePath)) { Write-Info "Normal.dotm-Quelle fehlt ($sourcePath), Ã¼berspringe."; return }
...
function Copy-ExcelTemplate {
  ...
  if (-not (Test-Path $sourcePath)) { Write-Info "Mappe.xltx-Quelle fehlt ($sourcePath), Ã¼berspringe."; return }
  New-EnsuredPath (Split-Path $targetPath -Parent)
  try {
    Stop-NamedProcess EXCEL
    if (Test-Path $targetPath) {
...
function New-CandidateFoldersFromExcel {
  ...
  try {
    ...
    Write-Info "Ordner fÃ¼r PrÃ¼fungskandidaten wurden angelegt."
  } catch {
    throw
  } finally {
    if ($wb) { 
...
function Copy-CandidateFolderToDesktop {
  ...
  if (-not (Test-Path $source)) { Write-Warning "Kein Kandidaten-Ordner fÃƒÂ¼r aktuellen Nutzer: $source"; return }
  try {
...
function Set-Proxy {
  ...
  if ($State -eq 'Skip') { Write-Info "Proxy-Konfiguration Ã¼bersprungen."; return }
...
try {
  Write-Host -ForegroundColor Yellow "Dieses Skript richtet den PrÃ¼fungsrechner fÃ¼r die AP 1 ein."
  ...
  if ($Nuera) {
    Write-Info "Suche und lade neueste Nuera/NÃ¼ra..."
    Get-LatestNueraFile
  }
  ...
  Write-Info "Ãœbernehme Autokorrektur-Einstellungen..."
  Set-WordAutoCorrectRegistry
  Set-ExcelAutoCorrectRegistry

  Write-Info "Ãœbernehme Schnellzugriff-Symbolleisten..."
  ...
  try {
    if (-not $RegistryOnly) {
      ...
      Write-Info "COM nicht verfÃ¼gbar â€“ nutze CSV-Fallback..."
      $rootPath = New-CandidateFoldersFromCsv -CsvPath $CsvFallbackPath -MaxRows $MaxRows
    } else {
      Write-Warning "Kein COM und kein CsvFallbackPath â€“ Ordnererzeugung wird Ã¼bersprungen."
    }
  } catch {
  ...
  if ($rootPath) {
    ...
    Write-Info "RÃ¤ume temporÃ¤ren Ordner auf..."
    try {
  ...
  # Taskbar und Proxy
  Write-Info "Taskbar-Einstellungen Ã¼bernehmen..."
  ...
  Write-Host -ForegroundColor Green 'Fertig. Der Rechner ist fuer die AP 1 vorbereitet.'
}
catch {
  Write-Host -ForegroundColor Red 'FEHLER beim Ausfuehren des Skripts'
}
finally {
  Stop-PrepTranscript
}
